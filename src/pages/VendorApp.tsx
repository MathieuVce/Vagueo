...
export default function VendorApp() {
  const [stand, { advance, togglePause, toggleOpen, setFlowRate, configure, claimStand }] =
    useStand({ autoCreate: true });
  const {
    user,
    loading: authLoading,
    isAuthorized,
  ...
  // ─── No stand ID in URL ───────────────────────────────────────
  if (!STAND_ID) {
    if (authLoading || standLookup === 'loading' || standLookup === 'redirecting')
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 13, color: p.mute, lineHeight: 1.6, maxWidth: 280 }}>
          Ce stand appartient à un autre compte.
          <br />
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 13, color: p.mute, lineHeight: 1.7, maxWidth: 300 }}>
          Votre stand a été créé et est en cours de validation par l'administrateur.
          <br />
          Vous serez notifié dès qu'il est approuvé.
        </div>
        <div style={{ marginTop: 8, fontSize: 11, color: p.mute, fontFamily: 'monospace' }}>
          {STAND_ID}
        </div>
        <button
          onClick={signOut}
