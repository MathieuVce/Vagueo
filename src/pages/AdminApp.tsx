...
function Avatar({ stand }: { stand: StandDoc }) {
  ...
  if (stand.logo_url) {
    return <img src={stand.logo_url} alt="" style={{ width: 48, height: 48, borderRadius: SIZE.r3, objectFit: 'cover', flexShrink: 0 }} />;
  }
  return (
    <div style={{ width: 48, height: 48, borderRadius: SIZE.r3, flexShrink: 0, background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 700, fontFamily: FONT.sans }}>
      {initial}
    </div>
  );
}
...
function Pill({ children, bg, color }: { children: React.ReactNode; bg?: string; color?: string }) {
  return (
    <span style={{
      padding: '3px 9px', borderRadius: SIZE.rFull,
      border: `1px solid ${COLOR.line}`,
      background: bg ?? 'transparent',
      ...TEXT.caption, color: color ?? COLOR.mute,
...
function StatusBadge({ active, label }: { active: boolean; label: string }) {
  return (
    <span style={{
      padding: '3px 8px', borderRadius: SIZE.r2,
      background: active ? COLOR.successBg : COLOR.line,
      color:      active ? COLOR.success    : COLOR.mute,
      ...TEXT.caption, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase',
    }}>
      {label}
    </span>
  );
...
function Metric({ label, value }: { label: string; value: number | string }) {
  return (
    <div style={{ flex: 1, textAlign: 'center' }}>
      ...
      <div style={{ ...TEXT.caption, color: COLOR.mute, textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: 2 }}>{label}</div>
    </div>
  );
}
...
function FlowTable({ slow, sprint }: { slow: number; sprint: number }) {
  return (
    <div style={{ borderRadius: SIZE.r3, border: `1px solid ${COLOR.line}`, overflow: 'hidden', marginTop: 10 }}>
      {[1, 2, 3, 4, 5].map(level => (
        <div key={level} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 12px', background: level % 2 === 0 ? 'transparent' : COLOR.surface, ...TEXT.small }}>
...
// ─── Stand Editor ─────────────────────────────────────────────────
function StandEditor({ stand, onClose, onDeleted }: { stand: StandDoc; onClose: () => void; onDeleted: () => void }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const update: Record<string, unknown> = {
        name: name.trim(), address: address.trim(), logo_url: logoUrl.trim(),
        is_open: isOpen, is_paused: isPaused,
        flow_slow: slow, flow_sprint: sprint,
  ...
  function copyLink() {
    navigator.clipboard.writeText(clientUrl).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); });
  }

  const pillBtn: CSSProperties = { padding: '6px 11px', border: `1px solid ${COLOR.line}`, borderRadius: SIZE.r2, background: 'transparent', color: COLOR.mute, fontFamily: FONT.sans, fontSize: 11, cursor: 'pointer' };

  const livePills = (
    <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
      {liveCount !== null && (
        <>
          <Pill><span style={{ fontFamily: FONT.mono, fontWeight: 700, color: COLOR.ink }}>{liveCount}</span>&nbsp;en file</Pill>
          <Pill><span style={{ fontFamily: FONT.mono, fontWeight: 700, color: COLOR.ink }}>{stand.queue_counter ?? 0}</span>&nbsp;total</Pill>
        </>
      )}
      {isClaimed
        ? <Pill bg={COLOR.successBg} color={COLOR.success}>Revendiqué</Pill>
        : <Pill bg={COLOR.warningBg} color={COLOR.warning}>Non revendiqué</Pill>
      }
    </div>
  );
  ...
  return (
    <>
      ...
      <Drawer
        onClose={onClose}
        header={<DrawerHeader title="Modifier le stand" subtitle={stand._id} onClose={onClose} extra={livePills} />}
      >
        <DrawerBody>
          {/* QR code */}
          <div>
            ...
            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', marginTop: 10 }}>
              <div style={{ padding: 12, borderRadius: SIZE.r4, background: '#fff', boxShadow: SHADOW.md, flexShrink: 0 }}>
                <QRCodeSVG value={clientUrl} size={96} level="M" fgColor={COLOR.ink} bgColor="#ffffff" />
              </div>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ ...TEXT.monoSm, color: COLOR.mute, wordBreak: 'break-all', lineHeight: 1.5 }}>{clientUrl}</div>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button onClick={copyLink} style={pillBtn}>{copied ? '✓ Copié' : 'Copier lien client'}</button>
                  <a href={vendorUrl} target="_blank" rel="noopener noreferrer" style={{ ...pillBtn, textDecoration: 'none', display: 'inline-flex' }}>URL vendeur →</a>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <Toggle on={isOpen}   onToggle={() => setIsOpen(v => !v)}   label={isOpen   ? 'File ouverte' : 'File fermée'}   sublabel={isOpen   ? 'Les clients peuvent rejoindre.' : 'Personne ne peut rejoindre.'} />
            <Toggle on={isPaused} onToggle={() => setIsPaused(v => !v)} label={isPaused ? 'File en pause' : 'File active'}  sublabel={isPaused ? "Avance automatique suspendue."  : 'Les vagues avancent automatiquement.'} />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <Field label="Adresse / emplacement" value={address} onChange={setAddress} placeholder="Marché des Capucins, Stand B12" max={100} />
            <Field label="URL du logo"           value={logoUrl} onChange={setLogoUrl} placeholder="https://…" type="url" />
          </div>
          ...
          <div>
            ...
            <div style={{ marginTop: 10 }}>
              {isClaimed ? (
                <div style={{ padding: '12px 14px', borderRadius: SIZE.r3, background: COLOR.surface, border: `1px solid ${COLOR.line}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ ...TEXT.caption, color: COLOR.mute, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Compte lié</div>
                      <div style={{ ...TEXT.h3, color: COLOR.ink }}>
                        {stand.vendor_email || <span style={{ ...TEXT.small, color: COLOR.mute, fontWeight: 400, fontStyle: 'italic' }}>email non enregistré</span>}
                      </div>
                      <div style={{ ...TEXT.monoSm, color: COLOR.mute, marginTop: 3, wordBreak: 'break-all' }}>{stand.vendor_uid}</div>
                    </div>
                    {!confirmUnlink ? (
                      <Button variant="danger" size="sm" onClick={() => setConfirmUnlink(true)}>Délier</Button>
                    ) : (
                      <div style={{ display: 'flex', gap: 6 }}>
                        <Button variant="danger" size="sm" onClick={handleUnlink}>Confirmer</Button>
                        <Button variant="ghost"  size="sm" onClick={() => setConfirmUnlink(false)}>Non</Button>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div style={{ ...TEXT.small, color: COLOR.mute, lineHeight: 1.5 }}>
                    Le vendeur se connecte sur <span style={{ fontFamily: FONT.mono }}>/vendor?stand={stand._id}</span> avec cet email → liaison automatique.
                  </div>
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
              <NumberField label="Rythme calme"   value={flowSlow}   unit="min" onChange={v => setFlowSlow(Math.max(flowSprint + 0.5, v))} />
              <NumberField label="Rythme sprint"  value={flowSprint} unit="min" onChange={v => setFlowSprint(Math.min(flowSlow - 0.5, Math.max(0.5, v)))} />
            </div>
          ...
          <div>
            ...
            <Segment
              ...
              options={[{ value: false, label: 'Illimitée' }, { value: true, label: 'Limitée' }]}
            />
            {limitQueue && <div style={{ marginTop: 8 }}><NumberField value={maxQueueSize} unit="personnes max" onChange={v => setMaxQueueSize(Math.max(1, v))} /></div>}
          </div>
          ...
          <div>
            ...
            <Segment
              ...
              options={[{ value: false, label: 'Illimité' }, { value: true, label: 'Limité' }]}
            />
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <Button variant="ghost" onClick={onClose} style={{ minWidth: 80 }}>Annuler</Button>
          </div>

          {/* Suppression */}
          <div style={{ paddingTop: 16, borderTop: `1px solid ${COLOR.line}` }}>
            {!confirmDel ? (
              <Button full variant="danger" size="sm" onClick={() => setConfirmDel(true)}>Supprimer ce stand…</Button>
            ) : (
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{ ...TEXT.small, color: COLOR.danger, flex: 1 }}>Confirmer la suppression ?</span>
                <Button variant="danger" size="sm" onClick={handleDelete}>Supprimer</Button>
                <Button variant="ghost"  size="sm" onClick={() => setConfirmDel(false)}>Non</Button>
              </div>
            )}
          </div>
        </DrawerBody>
...
// ─── Create Stand Modal ───────────────────────────────────────────
function CreateStandModal({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  async function handleCreate() {
    ...
    const ref = await addDoc(collection(db, 'stands'), {
      ...
      current_wave: 0, queue_counter: 0, is_paused: false, is_open: false,
      flow_rate: FLOW_RATE_DEFAULT, flow_slow: slow, flow_sprint: sprint,
  ...
  return (
    <Drawer onClose={onClose} header={<DrawerHeader title="Nouveau stand" onClose={onClose} />}>
      <DrawerBody>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Label>Identité</Label>
          <Field label="Nom du stand *"          value={name}    onChange={setName}    placeholder="Churros Mathieu…" max={60} />
          <Field label="Adresse / emplacement"   value={address} onChange={setAddress} placeholder="Marché des Capucins, Stand B12" max={100} />
          <Field label="URL du logo"             value={logoUrl} onChange={setLogoUrl} placeholder="https://…" type="url" />
        </div>
        ...
        <div>
          <Label>Capacité de la file</Label>
          <Segment style={{ marginTop: 8 }} value={limitQueue} onChange={setLimitQueue} options={[{ value: false, label: 'Illimitée' }, { value: true, label: 'Limitée' }]} />
          {limitQueue && <div style={{ marginTop: 8 }}><NumberField value={maxQueueSize} unit="personnes max" onChange={v => setMaxQueueSize(Math.max(1, v))} /></div>}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <Button full variant="primary" onClick={handleCreate} disabled={!canCreate} loading={saving}>Créer le stand</Button>
          <Button variant="ghost" onClick={onClose} style={{ minWidth: 80 }}>Annuler</Button>
        </div>
      </DrawerBody>
    </Drawer>
  ...
  return (
    <div style={{ background: COLOR.paper, border: `1px solid ${COLOR.line}`, borderRadius: SIZE.r5, padding: '18px 20px', fontFamily: FONT.sans }}>
      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
        <Avatar stand={stand} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ ...TEXT.h2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {stand.name || <span style={{ color: COLOR.mute, fontStyle: 'italic', fontWeight: 400 }}>Sans nom</span>}
          </div>
          {stand.address && (
            <div style={{ ...TEXT.small, color: COLOR.mute, marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              📍 {stand.address}
            </div>
          )}
          <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
            ...
            <span style={{ padding: '3px 8px', borderRadius: SIZE.r2, background: COLOR.line, ...TEXT.caption, color: COLOR.mute, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
              {flowLabel}
            </span>
            {stand.max_queue_size != null && (
              <span style={{ padding: '3px 8px', borderRadius: SIZE.r2, background: COLOR.line, ...TEXT.caption, color: COLOR.mute }}>
                max {stand.max_queue_size}
              </span>
            )}
            {!stand.vendor_uid && (
              <span style={{ padding: '3px 8px', borderRadius: SIZE.r2, background: COLOR.warningBg, ...TEXT.caption, color: COLOR.warning, fontWeight: 600 }}>
                Non revendiqué
              </span>
            )}
          </div>
...
function AdminLogin({ onSignIn, error }: { onSignIn: () => void; error: string | null }) {
  return (
    <div style={{ width: '100%', height: '100%', background: COLOR.paper, fontFamily: FONT.sans, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, animation: 'vagueoFadeIn 0.3s ease' }}>
      <VagueoLogo size={48} color={COLOR.ink} accent={COLOR.primary} />
      <div style={{ marginTop: 16, fontFamily: FONT.serif, fontStyle: 'italic', fontSize: 32, letterSpacing: '-0.02em' }}>Administration</div>
      <div style={{ marginTop: 8, ...TEXT.small, color: COLOR.mute, textAlign: 'center', maxWidth: 280, lineHeight: 1.6 }}>
        Connectez-vous avec votre compte Google pour accéder au tableau de bord.
      </div>
      {error && (
        <div style={{ marginTop: 16, padding: '10px 16px', borderRadius: SIZE.r3, background: COLOR.dangerBg, color: COLOR.danger, ...TEXT.small }}>{error}</div>
      )}
      <button onClick={onSignIn} style={{
        marginTop: 32, padding: '14px 28px', border: 0, cursor: 'pointer',
        background: COLOR.ink, color: COLOR.paper, borderRadius: SIZE.r4,
...
export default function AdminApp() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(collection(db, 'stands'), snap => {
      setStands(snap.docs.map(d => ({ _id: d.id, ...(d.data() as Omit<Stand, 'secure_color'>) } as StandDoc)));
    });
  }, [user]);
  ...
  if (loading) {
    return (
      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: COLOR.paper }}>
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: COLOR.primary, animation: 'vagueoPulse 1s ease-in-out infinite' }} />
      </div>
    );
  }
  ...
  if (adminEmail && user.email !== adminEmail) {
    return (
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: COLOR.paper, fontFamily: FONT.sans, gap: 12 }}>
        <div style={{ ...TEXT.h3 }}>Accès refusé</div>
        <div style={{ ...TEXT.small, color: COLOR.mute }}>Ce compte n'est pas autorisé.</div>
        <Button variant="ghost" size="sm" onClick={signOut} style={{ marginTop: 8 }}>Se déconnecter</Button>
      </div>
    );
  }
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: COLOR.surface, fontFamily: FONT.sans, overflowY: 'auto' }}>
      {/* Header */}
      <div style={{ position: 'sticky', top: 0, zIndex: 10, background: COLOR.paper, borderBottom: `1px solid ${COLOR.line}`, padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '32px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, gap: 12 }}>
          <div>
            <div style={{ fontFamily: FONT.serif, fontStyle: 'italic', fontSize: 30, letterSpacing: '-0.02em' }}>Stands</div>
            <div style={{ ...TEXT.small, color: COLOR.mute, marginTop: 2 }}>
              {stands.length} stand{stands.length !== 1 ? 's' : ''} enregistré{stands.length !== 1 ? 's' : ''}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {stands.length > 3 && (
              <input
                ...
                style={{ padding: '8px 14px', border: `1.5px solid ${COLOR.line}`, borderRadius: SIZE.r3, background: COLOR.paper, fontFamily: FONT.sans, fontSize: 13, color: COLOR.ink, outline: 'none', width: 180 }}
              />
            )}
            <Button variant="primary" size="sm" onClick={() => setShowCreate(true)}>+ Nouveau stand</Button>
          </div>
        </div>

        {/* Toast: new stand created */}
        {newId && (
          <div style={{ marginBottom: 16, padding: '12px 16px', borderRadius: SIZE.r3, background: COLOR.successBg, border: `1px solid ${COLOR.successMid}`, ...TEXT.small, color: COLOR.success, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>Stand créé — ID&nbsp;<span style={{ fontFamily: FONT.mono, fontWeight: 700 }}>{newId}</span></span>
            <button onClick={() => setNewId(null)} style={{ border: 0, background: 'transparent', cursor: 'pointer', color: COLOR.success, fontSize: 16, padding: 0 }}>×</button>
          </div>
        )}
