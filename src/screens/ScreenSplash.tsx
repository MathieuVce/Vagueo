...
const s = {
  ...
  tagline: {
    marginTop: 4, fontSize: 11, color: p.mute,
    letterSpacing: '0.14em', textTransform: 'uppercase',
  } satisfies CSSProperties,
  taglineNoName: {
    marginTop: 12, fontSize: 12, color: p.mute,
  ...
  closedBadge: {
    ...
    background: p.line, color: p.mute,
    fontSize: 13, fontWeight: 500,
  } satisfies CSSProperties,
  closedHint: {
    fontSize: 13, color: p.mute, lineHeight: 1.6, maxWidth: 260,
  } satisfies CSSProperties,
  waitBlock: {
    marginTop: 48, textAlign: 'center',
  } satisfies CSSProperties,
  ...
  waitUnit: {
    fontSize: 22, color: p.mute, marginLeft: 6,
  } satisfies CSSProperties,
  bottom: {
    padding: '0 22px 40px',
  ...
  footnote: {
    fontSize: 11, color: p.mute, textAlign: 'center', lineHeight: 1.5,
  } satisfies CSSProperties,
  poweredBy: {
    fontSize: 11, color: p.mute, textAlign: 'center', lineHeight: 1.5,
  } satisfies CSSProperties,
...
export default function ScreenSplash({
  onJoin, estimatedMin = 15, standName, logoUrl, isOpen = true, isPaused = false, isFull = false,
}: ScreenSplashProps) {
        ...
        {isUnavailable ? (
          <div style={s.closedBlock}>
            ...
            <div style={s.closedHint}>
              {isPaused
                ? <>Le vendeur a momentanément suspendu la file.<br />Revenez dans quelques instants.</>
                : isFull
                  ? <>La file a atteint sa capacité maximale.<br />Revenez dans quelques instants.</>
                  : <>Le stand n'est pas encore disponible.<br />Revenez dans quelques instants.</>}
            </div>
          </div>
        ) : (
        ...
        {!isUnavailable ? (
          <>
            ...
            <div style={s.footnote}>
              Pas d'inscription, pas de compte.<br />
