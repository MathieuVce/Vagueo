...
<div class="empty">Aucun passage enregistré pour cette période.</div>
  ...
  const delayRow = (s.delayUsed ?? 0) > 0
    ? `<tr><td>Délais 10 min utilisés</td><td class="pct"></td><td class="num">${s.delayUsed}</td></tr>`
    : '';

  const timeCards = [
    s.avgWaitMin    != null ? `<div class="card"><div class="val">~${s.avgWaitMin}</div><div class="lbl">min d'attente moy.</div></div>`  : '',
    s.avgServiceMin != null ? `<div class="card"><div class="val">~${s.avgServiceMin}</div><div class="lbl">min au stand moy.</div></div>` : '',
  ].join('');
...
export default function ScreenStats({ onClose }: ScreenStatsProps) {
  ...
  return (
    <div style={{
      ...
      {/* Period tabs */}
      <div style={{
        margin: '14px 22px 0', flexShrink: 0,
        display: 'flex', gap: 5,
        background: 'rgba(17,20,26,0.05)', borderRadius: 12, padding: 4,
      }}>
        {PERIODS.map(t => (
          <button key={t.key} onClick={() => setPeriod(t.key)} style={{
            flex: 1, border: 0, cursor: 'pointer', borderRadius: 9,
            padding: '7px 4px',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px 48px' }}>
        {error ? (
          <div style={{ textAlign: 'center', paddingTop: 64 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#c0392b', marginBottom: 8 }}>Erreur de chargement</div>
            <div style={{ fontSize: 12, color: p.mute, maxWidth: 280, margin: '0 auto', lineHeight: 1.6, wordBreak: 'break-all' }}>
              {error}
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', paddingTop: 56 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: p.wait, animation: 'vagueoPulse 1s ease-in-out infinite' }} />
          </div>
        ) : stats.total === 0 ? (
          <div style={{ textAlign: 'center', paddingTop: 64 }}>
            ...
            <div style={{ fontSize: 13, color: p.mute, lineHeight: 1.6, maxWidth: 240, margin: '0 auto 28px' }}>
          ...
          <>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <StatCard value={stats.served ?? 0}                  label="Servis"    mono big color="oklch(0.42 0.16 142)" />
              <StatCard value={(stats.total ?? 0) - (stats.served ?? 0)} label="Absences" mono big color={p.mute} />
            </div>
            ...
            <div style={{ marginTop: 12, display: 'flex', borderRadius: 8, overflow: 'hidden', height: 7 }}>
              ...
              <div style={{ flex: (stats.leftWaiting ?? 0) + (stats.leftCheckin ?? 0),             background: p.call }} />
              <div style={{ flex: (stats.timeoutCheckin ?? 0) + (stats.timeoutService ?? 0),       background: p.line }} />
            </div>
            ...
            {(stats.delayUsed ?? 0) > 0 && (
              <div style={{
                ...
                <span style={{ fontFamily: FONT_MONO, fontWeight: 700, color: p.ink }}>{stats.delayUsed}</span>
              </div>
            )}
              ...
              <div style={{ marginTop: 28 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: BAR_H }}>
                  {hd.hours.map(h => {
                    ...
                    const barH  = count > 0 ? Math.max(6, Math.round(count / hd.maxCnt * BAR_H)) : 2;
                    return (
                      <div key={h} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: BAR_H }}>
                        {count > 0 && <div style={{ fontSize: 8, color: p.mute, marginBottom: 2 }}>{count}</div>}
                        <div style={{ width: '100%', height: barH, background: count > 0 ? p.wait : p.line, borderRadius: '3px 3px 0 0' }} />
                      </div>
                    );
                  })}
                </div>
                <div style={{ display: 'flex', gap: 3, marginTop: 4 }}>
                  {hd.hours.map((h, i) => (
                    <div key={h} style={{ flex: 1, fontSize: 9, textAlign: 'center', color: p.mute, opacity: i % 2 === 0 ? 1 : 0 }}>
                      {h}h
                    </div>
                  ))}
                </div>
            ...
            <div style={{ marginTop: 28, paddingTop: 20, borderTop: `1px solid ${p.line}` }}>
              <div style={{ fontSize: 11, color: p.mute, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 10 }}>
                Détail des sorties
              </div>
              <ExitRow label="Servis (passés au stand)"   value={stats.served ?? 0}           total={stats.total} accent="oklch(0.42 0.16 142)" />
              <ExitRow label="Partis en attente"          value={stats.leftWaiting ?? 0}      total={stats.total} />
              <ExitRow label="Partis lors de l'appel"    value={stats.leftCheckin ?? 0}      total={stats.total} />
...
function StatCard({ value, label, sublabel, color, big, mono }: StatCardProps) {
  ...
  return (
    <div style={{
      flex: 1, padding: big ? '14px 10px' : '12px 10px',
      ...
      <div style={{ fontSize: 9, color: p.mute, marginTop: 3, textTransform: 'uppercase', letterSpacing: '0.1em', lineHeight: 1.4 }}>
        {label}
      </div>
      {sublabel && (
...
function ExitRow({ label, value, total, accent }: ExitRowProps) {
  ...
  return (
    <div style={{
      ...
      padding: '9px 0', borderBottom: `1px solid ${p.line}`, fontSize: 13,
    }}>
      <span style={{ color: p.mute }}>{label}</span>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0 }}>
        <span style={{ fontSize: 11, color: p.mute }}>{pct}%</span>
        <span style={{ fontFamily: FONT_MONO, fontWeight: 700, color: accent ?? p.ink, minWidth: 22, textAlign: 'right' }}>{value}</span>
      </div>
    </div>
  );
}
