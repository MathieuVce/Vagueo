...
const s = {
  ...
  openBtn: (is_open: boolean): CSSProperties => ({
    ...
    cursor: 'pointer',
    width: '100%',
    minHeight: is_open ? 44 : 64,
    background: is_open ? p.line : 'oklch(0.38 0.18 142)',
    color: is_open ? p.mute : '#fff',
    borderRadius: 16,
    fontFamily: FONT,
    fontSize: is_open ? 13 : 17,
    fontWeight: is_open ? 400 : 600,
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 8,
    ...
    <div style={s.root}>
      ...
      <div style={s.controls}>
        ...
        <div style={s.flowControl}>
          <button
            onClick={() => onSetFlowRate(-1)}
            disabled={flow_rate <= 1}
            style={s.flowBtn(flow_rate <= 1)}
          >
            −
          </button>
          ...
          <button
            onClick={() => onSetFlowRate(+1)}
            disabled={flow_rate >= 5}
            style={s.flowBtn(flow_rate >= 5)}
          >
            +
          </button>
        </div>
      ...
      {isDevMode && (
        <div style={s.devToolbar}>
          ...
          {(
            ...
            ({ label, fn }) =>
              fn && (
                <button key={label} onClick={fn} style={s.devBtn}>
                  {label}
                </button>
              ),
          )}
          {onDevResetStore && (
            <button onClick={onDevResetStore} style={s.devResetBtn}>
              ↺ Reset store
            </button>
          )}
        </div>
