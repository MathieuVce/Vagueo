...
const s = {
  root: {
    ...
    background: COLOR.paper, fontFamily: FONT.sans,
  ...
  title: {
    ...TEXT.display, fontSize: 36,
  } satisfies CSSProperties,
  subtitle: {
    ...TEXT.small, color: COLOR.mute, marginTop: 8,
  } satisfies CSSProperties,
  logoSection: {
    marginTop: 36,
  } satisfies CSSProperties,
  logoRow: {
    marginTop: 10, display: 'flex', gap: 14, alignItems: 'center',
  } satisfies CSSProperties,
  logoPreview: {
    width: 72, height: 72, borderRadius: SIZE.r5, flexShrink: 0,
    border: `1.5px solid ${COLOR.line}`, overflow: 'hidden',
  ...
  logoError: {
    ...TEXT.small, color: COLOR.danger, marginTop: 6,
  } satisfies CSSProperties,
  fieldGap: {
    marginTop: 28,
  } satisfies CSSProperties,
  ...
  flowHint: {
    ...TEXT.small, color: COLOR.mute, marginTop: 4,
  } satisfies CSSProperties,
  flowRow: {
    marginTop: 14, display: 'flex', gap: 12,
  } satisfies CSSProperties,
  flowTable: {
    marginTop: 14, borderRadius: SIZE.r3,
  ...
  required: {
    ...TEXT.small, color: COLOR.mute, textAlign: 'center', marginTop: 8,
  } satisfies CSSProperties,
};

export default function ScreenVendorSetup({ stand, onSave, isEditing = false }: ScreenVendorSetupProps) {
  ...
  return (
    <div style={s.root}>
      ...
      <div style={s.header}>
        ...
        {isEditing && (
          <Button variant="ghost" size="sm" onClick={() => onSave(null)}>Annuler</Button>
        )}
      </div>
      ...
      <div style={s.form}>
          ...
          <div style={s.logoRow}>
            <div style={s.logoPreview}>
              {logoUrl && !imgError ? (
                ...
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={COLOR.mute} strokeWidth="1.5">
            ...
            <div style={s.logoUrlWrap}>
              <Field
                ...
                value={logoUrl}
                onChange={(v) => { setLogoUrl(extractImageUrl(v)); setImgError(false); }}
              />
            </div>
          </div>
          {logoUrl && imgError && (
        ...
        <div style={s.flowSection}>
          ...
          <div style={s.flowHint}>
            Temps moyen pour servir une personne selon l'affluence.
          </div>
          <div style={s.flowRow}>
            <NumberField
            ...
            <NumberField
              ...
              value={flowSprint}
              onChange={(v) => setFlowSprint(+Math.min(flowSlow - 0.5, Math.max(0.5, v)).toFixed(1))}
              min={0.5} step={0.5}
              unit="min/pers"
            />
          </div>
          ...
          <div style={s.flowTable}>
            {[1, 2, 3, 4, 5].map((level) => (
              <div key={level} style={s.flowTableRow(level % 2 === 0)}>
                ...
                <span style={{ ...TEXT.small, fontWeight: level === 1 || level === 5 ? 600 : 400, color: COLOR.ink }}>
        ...
        <div style={s.fieldGap}>
          ...
          <div style={s.toggleWrap}>
            <Toggle
              ...
              label={isOpen ? 'Ouverte — les clients peuvent rejoindre' : 'Fermée — QR code inactif'}
              sublabel={isOpen
      ...
      <div style={s.footer}>
        ...
        {!canSave && (
          <div style={s.required}>Le nom du stand est requis.</div>
        )}
      </div>
    </div>
  );
